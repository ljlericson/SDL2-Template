...
{
Script::Script(const std::string& file) : m_file(file)
{
    sm_lua.open_libraries(sol::lib::base, sol::lib::math, sol::lib::table, sol::lib::string,
                          sol::lib::debug);
    // 1. Load Lua file
    ...
    {
        Console::ccout << "[Lua] Script did not return a function: " << file << std::endl;
        auto [begin, end] = Console::cchat.getMessageIterators();
        size_t elem = std::distance(begin, end) - 1;
        Console::Message& mes = Console::cchat.getMessage(elem);
        mes.color = ImVec4(255.0f, 0.0f, 0.0f, 255.0f);
