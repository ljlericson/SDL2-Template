...
namespace App
{
StateManager::StateManager(EventSystem::EventDispatcher& eventDispatcher,
                           LayerSystem::LayerStack& layerStack, Core::SDLBackend::Renderer& r,
                           Core::SDLBackend::Window& w)
    : mr_eventDispatcher(eventDispatcher), mr_layerStack(layerStack), mr_window(w), mr_renderer(r)
{
...
void StateManager::onInput(const bool* keyboardState, EventType e,
                           const std::vector<uint32_t>& events)
{
    // settings logic
    (void)events; // stop that compiler warning message
    ...
    if (mr_layerStack.layerActive<LayerSystem::GameLayer>())
    {
        ...
        if (escJustPressed)
        {
            ...
            {
                mr_layerStack.pushLayer<LayerSystem::SettingsLayer>(mr_eventDispatcher,
                                                                    mr_renderer);
    ...
    case EventType::roundEnd:
    {
        ...
        if (result)
            mr_layerStack.pushLayer<LayerSystem::ShopLayer>(
                mr_eventDispatcher, result.value().get().getModifierManager(), mr_renderer);
        else
