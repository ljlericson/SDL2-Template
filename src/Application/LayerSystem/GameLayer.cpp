...
{
GameLayer::GameLayer(EventSystem::EventDispatcher& eventDispatcher,
                     const Core::SDLBackend::Window& window,
                     const Core::SDLBackend::Renderer& renderer, std::string_view savePath)
    : mr_eventDispatcher(eventDispatcher)
{
    ...
    m_gameplayManager = std::make_unique<GameComponents::GameplayManager>(
        eventDispatcher, renderer, *m_modifierManager, *m_scrabbleBoard,
        m_scrabbleBoard->getNumTiles(), 21);
    m_button = std::make_unique<UIComponents::Button>(
        renderer, SDL_FRect{.x = 1280 - 200, .y = 200, .w = 111.0f, .h = 55.0f}, "Dummy Button");

    m_backgroundTex = Core::AssetManager::textureManager->newTexture(
        "background", renderer.getRendHand(), "./assets/Textures/GameComponents/Background.png");
...
void GameLayer::render(const Core::SDLBackend::Renderer& renderer)
{
    ...
    // background texture
    renderer.render(*m_backgroundTex, SDL_FRect(0.0f, 0.0f, (float)Utils::getWindowSize().first,
                                                (float)Utils::getWindowSize().second));
    // game components
    m_scrabbleBoard->render(renderer);
    m_gameplayManager->render(
        renderer, m_active ? GameComponents::GameplayManager::UpdateGameSystemOnRender::true_
                           : GameComponents::GameplayManager::UpdateGameSystemOnRender::false_);
    m_button->render(renderer);

    if (m_active)
        ImGui::Begin("CONTROLS");
    else
        ImGui::Begin("CONTROLS", nullptr, ImGuiWindowFlags_NoInputs);
    ...
    if (ImGui::BeginPopupModal("Dev Mode", NULL, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        if (ImGui::InputText("##Password", &input,
