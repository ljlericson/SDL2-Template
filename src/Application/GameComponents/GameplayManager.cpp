...
{
GameplayManager::GameplayManager(EventSystem::EventDispatcher& eventDispatcher,
    ...
    : m_highlighter(SDL_Color{.r = 0, .g = 0, .b = 255, .a = 100}, numTiles),
      m_numTilesLeft(numTilesPerGame), m_numTilesTotal(numTilesPerGame), mr_renderer(renderer),
      mr_eventDispatcher(eventDispatcher), mr_modifierManager(modifierManager), mr_board(board),
      m_numTiles(numTiles),
      m_scoreText(glm::vec2(Utils::getWindowSize().first - 10.0f, 0.0f), 20, 20,
                  "./assets/Fonts/font.ttf", SDL_Color(0, 255, 0, 255), "Points: "),
      m_tilesLeftText(glm::vec2(Utils::getWindowSize().first - 10.0f, 20.0f), 20, 20,
                      "./assets/Fonts/font.ttf", SDL_Color(255, 0, 0, 255), "Number of Tiles: "),
      m_targetScoreText(glm::vec2(Utils::getWindowSize().first - 10.0f, 40.0f), 20, 20,
                        "./assets/Fonts/font.ttf", SDL_Color(235, 201, 52, 255), "Target Points: "),
      m_tileSlots({false, false, false, false, false, false, false}), m_tileRecycler(renderer)
...
void GameplayManager::render(const Core::SDLBackend::Renderer& renderer,
                             UpdateGameSystemOnRender renderType)
{
    ...
    for (size_t i = 0; i < m_activeTiles.size(); ++i)
    {
        ...
        if (renderType == UpdateGameSystemOnRender::true_)
        {
            ...
            else if (result == GameComponents::Tile::PressState::pressed)
            {
                ...
                {
                    auto it =
                        std::find_if(m_activeTiles.begin(), m_activeTiles.end(), [&](const auto& t)
                ...
                size_t tileIndex = mr_board.getSnapTileIndex(
                    {tile->pos.x + ((h / numTiles / 2.0f)), tile->pos.y + ((h / numTiles / 2.0f))});
    ...
    for (size_t badTileIndex : m_badWordIndexes)
        m_highlighter.render(renderer, badTileIndex);

    m_scoreText.render(renderer);

    m_tilesLeftText.setText("Number of Tiles: ");
    m_tilesLeftText.setText(
        m_tilesLeftText.getText().append(std::to_string(m_tiles.size() - m_nextTileIndex)));
    m_tilesLeftText.render(renderer);

    m_targetScoreText.render(renderer);
}

void GameplayManager::onInput(const bool* keyboardState, EventType e,
                              const std::vector<uint32_t>& events)
{
    ...
    case EventType::roundStart:
        ...
        for (int i = 0; i < m_numTilesLeft; i++)
        {
            m_tileRecycler.hide();
            char c;
            ...
            if (chance == 0)
            {
                c = mj_vowelsAndCons["vowels"]
                        .at(Utils::getRandomInt(0, (int32_t)mj_vowelsAndCons["vowels"].size() - 1))
                        .get<std::string>()[0];
            }
            else
            {
                c = mj_vowelsAndCons["consonants"]
                        .at(Utils::getRandomInt(0,
                                                (int32_t)mj_vowelsAndCons["consonants"].size() - 1))
                        .get<std::string>()[0];
            }
    ...
    case EventType::roundEnd: // ----------------
        ...
        if (!m_devMode && (m_score < m_targetScore))
        {
    ...
    case EventType::wordConfirmed: // ----------------
...

        {
            ...
            if (wordsOnBoard.size() > 0)
                m_score += mr_modifierManager.getBonusPoints(wordsOnBoard, baseScore, "wordScored",
                                                             m_numTilesLeft);
        }
