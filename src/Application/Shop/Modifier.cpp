...
Modifier::Modifier(const std::unordered_map<StaticModifierType, float>& staticModifiers,
                   bool stackable, std::unique_ptr<LuaScripting::Script> script)
    : m_staticModifiers(staticModifiers), m_stackable(stackable)
{
    if (script)
...
auto Modifier::stringToStaticModifier(const std::string& str)
    -> std::expected<StaticModifierType, std::string>
{
    if (str == "globalPriceReduction")
        ...
        return std::unexpected(std::string("No enum found for string \"") + str +
                               std::string("\""));
}
} // namespace Shop
} // namespace App
